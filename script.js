...
const dimension = 2 * radius + 2 * padding,
  points = 50,
  percentage = 0.62;
const angle = d3.scale.linear().domain([0, points - 1]).range([0, radians]);
const line = d3.svg.line.radial().interpolate("basis").tension(0).radius(radius).angle((d, i) => {
  if (i < points * percentage + 1) {
    return angle(i);
  }
});
const svg = d3.select(".outer-circle").append("svg").attr("width", dimension).attr("height", 200).append("g");
...
const canvas = d3.select(".inner-circle").append("svg").attr("height", r + 100).attr("width", r + 150);
const group = canvas.append("g").attr("transform", "translate(" + 120 + "," + 110 + ")");
const arc = d3.svg.arc().innerRadius(r / 1.2).outerRadius(r);

//returns objects based on data
const pie = d3.layout.pie().sort(null).value(data => data).startAngle(-110 * (pi / 180)).endAngle(110 * (pi / 180));
const arcs = group.selectAll(".arc").data(pie(data.lower)).enter().append("g").attr("class", "arc");
const defs = canvas.append("defs").append("linearGradient").attr("id", "greenGradient").attr("gradientUnits", "objectBoundingBox").attr("x1", "0").attr("y1", "0").attr("x2", "1").attr("y2", "1");
defs.append("stop").attr("offset", "0%").attr("stop-color", green1);
defs.append("stop").attr("offset", "100%").attr("stop-color", green2);
let path = arcs.append("path").attr("class", (data, index) => "progress-color" + index).attr("d", arc);
